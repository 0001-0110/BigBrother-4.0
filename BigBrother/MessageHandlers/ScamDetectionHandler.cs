...
namespace BigBrother.MessageHandlers
{
    internal class ScamDetectionHandler : MessageHandler
    {
        ...
        private readonly string _prompt = "You are in charge of detecting scams and fraudulent acts. A user will send you a message. You have to answer yes if the message is clearly a scam, and no otherwise. When in doubt, say no.";
        ...
        public override async Task Handle(SocketMessage message)
        {
            ...
            if (reply != null && reply.ToLower().Contains("yes"))
            {
                ...
                await _logger.LogVerbose($"{message.Content} was detected as a potential scam");
